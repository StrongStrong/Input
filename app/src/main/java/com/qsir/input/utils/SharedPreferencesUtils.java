package com.qsir.input.utils;

import android.content.Context;
import android.content.SharedPreferences;

import com.qsir.input.InputApplication;

public class SharedPreferencesUtils {
    private SharedPreferences share;
    private SharedPreferences.Editor editor;
    private String SHARED_NAME = "input";//sp的文件名 自定义


    //MyAPP.getContext() 是你的Application里面的一个Context
    private SharedPreferencesUtils() {
        share = InputApplication.getContext().getSharedPreferences(SHARED_NAME, Context.MODE_PRIVATE);
        editor = share.edit();
    }
...
//建议 所有的Key 以为常量的形式保存在此类里面
    ...
    public void putPanelHeight(int height) {
        putInt("input_method_height", height);
    }

    public int getPanelHeight() {
        return getInt("input_method_height", 0);
    }
